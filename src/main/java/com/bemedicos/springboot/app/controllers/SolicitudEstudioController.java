...
public class SolicitudEstudioController {
	...
	public String listarNotas(HttpServletRequest request, @PathVariable(value = "id") Long id, Map<String, Object> model,
			Model m) {
		...
		per = personaService.findOne(Long.valueOf(med.getPersona().getPersona_id()));
		System.out.println(per + " SDFGHJKJHGF " + per.getPersona_nombre());
		if (id > 0) {
